  ...
  class EventEvaluation
    ...
    def over_quota(user_id)
      ...
      GroupUser.where(user_id: user_id).each do |gu|
        ...
        if SiteSetting.chatbot_entry_level_trust_groups.split('|').include? gu.group_id.to_s
          max_quota = SiteSetting.chatbot_quota_entry_level_trust if max_quota < SiteSetting.chatbot_quota_entry_level_trust
        end
      end
      ...
      max_quota = SiteSetting.chatbot_quota_entry_level_trust if SiteSetting.chatbot_entry_level_trust_groups.split('|').include?("0") && max_quota < SiteSetting.chatbot_quota_entry_level_trust
